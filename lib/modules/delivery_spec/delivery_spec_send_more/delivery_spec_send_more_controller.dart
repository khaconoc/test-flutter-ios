...
class DeliverySpecSendMoreController extends GetxController {
  DeliverySpecRepository deliverSpecRepository;
  CommonRepository commonRepository;

  PositionService positionService = Get.find();
  DeliverySpecController deliverySpecController = Get.find();

  DeliverySpecSendMoreController({this.deliverSpecRepository, this.commonRepository});
  ...
  getData() async {
    ...
    var rs = await deliverSpecRepository.findOneBuuGuiMore(body: params);
  ...
  onLoadPagingItemList({page = 1, size = kSize}) async {
    ...
    var rs = await deliverSpecRepository.getPagingItemInDelivery(params);
  ...
  void onPhat() async {
    ...
    var rs = await deliverSpecRepository.onPhatMore(params);
    isSubmit.value = false;
    if (rs.ok()) {
